...
def create_module_file(package: str | None, basename: str, opts: Any,
    ...
    context['filename'] = basename.split('.')[-1]

    if g.args.import_parser:
        import_parser(basename, context, parent_context)
    else:
        parent_context['submodule_desc'].append({'module': basename, 'post': ''})
    
    text = ReSTRenderer(template_path).render('module.rst_t', context)
    write_file(qualname, text, opts)


def import_parser(basename, context, parent_context):
    def parser_importer():
        try:
            mod = __import__(basename, fromlist=[None])
            if hasattr(mod, 'get_parser'):
                descs.append({'module': basename, 'post': mod.get_parser().description})
                context['has_parser'] = True
                return
            descs.append({'module': basename, 'post': ''})
            context['has_parser'] = False
            return
    ...
    descs = parent_context['submodule_desc']
    th = threading.Thread(target=parser_importer, daemon=True)
    th.start()
    time_start = time.time()
    print(f"[APIDOC] Importing {basename}")
    while True:
        if not th.is_alive():
            break
        if time.time() - time_start > 1:
            break
        time.sleep(0.1)
    if 'has_parser' not in context:
        context['errored'] = True
        g.errors.append(basename)
        prRed(f"[APIDOC ERROR] {basename} has caused an error when imported")
...
def create_package_file(root: str, master_package: str | None, subroot: str,
    ...
    context['filename'] = pkgname.split('.')[-1]
    context['submodule_desc'] = []
    context['fancy_toc'] = True

    if submodules and opts.separatemodules:
        for submodule in submodules:
            create_module_file(None, submodule, opts, user_template_dir, context)
        ...
        if is_pkg or is_namespace:
            ...
            if subs or len(files) > 1 or not is_skipped_package(root, opts):
                ...
                if not is_namespace or has_child_module(root, excludes, opts):
                    if g.args.import_parser:
                        print(f"[APIDOC] Inserting new path {root}")
            ...
            for py_file in files:
                if not is_skipped_module(path.join(rootpath, py_file), opts, excludes):
                    ...
                    create_module_file(root_package, module, opts, user_template_dir, None)
                    toplevels.append(module)

    if len(g.errors) > 0:
        prRed('[APIDOC ERROR] failed to import files:')
...
Note: By default this script will not overwrite already created files."""))
    ...
    parser.add_argument('-PR', '--import-parser', action='store_true', dest='import_parser',
                        help=__('Import and include parser'))
...
def main(argv: Sequence[str] = (), /) -> int:
    ...
    print(f"Time taken {time.time() - g.t0}")
    return 0
